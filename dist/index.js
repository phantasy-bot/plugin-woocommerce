import { BasePlugin } from "@phantasy/plugin-sdk";
function asRecord(args) {
    return args && typeof args === "object" ? args : {};
}
function readPluginConfig(context) {
    const meta = context?.metadata?.pluginConfig;
    const fromMeta = meta && typeof meta === "object" ? meta : {};
    ...
    const environments = {
        local: {
            ...
            consumerKey: fromMeta.environments?.local?.consumerKey ??
                env?.WC_LOCAL_CONSUMER_KEY ??
                env?.WC_CONSUMER_KEY,
            consumerSecret: fromMeta.environments?.local?.consumerSecret ??
                env?.WC_LOCAL_CONSUMER_SECRET ??
                env?.WC_CONSUMER_SECRET,
        },
        staging: {
            baseUrl: fromMeta.environments?.staging?.baseUrl ??
                env?.WC_STAGING_BASE_URL ??
                fromMeta.baseUrl,
            consumerKey: fromMeta.environments?.staging?.consumerKey ??
                env?.WC_STAGING_CONSUMER_KEY ??
                fromMeta.consumerKey,
            consumerSecret: fromMeta.environments?.staging?.consumerSecret ??
                env?.WC_STAGING_CONSUMER_SECRET ??
                fromMeta.consumerSecret,
        },
        production: {
            baseUrl: fromMeta.environments?.production?.baseUrl ?? env?.WC_PRODUCTION_BASE_URL,
            consumerKey: fromMeta.environments?.production?.consumerKey ??
                env?.WC_PRODUCTION_CONSUMER_KEY,
            consumerSecret: fromMeta.environments?.production?.consumerSecret ??
    ...
    return {
        ...
        defaultEnvironment: fromMeta.defaultEnvironment ??
            env?.WC_DEFAULT_ENV ??
            "staging",
        environments,
    };
}
function resolveEnv(config, requested) {
    ...
    if (!creds.baseUrl) {
        throw new Error(`WC ${name} is not configured (set environments.${name} or WC_${name.toUpperCase()}_* env)`);
    }
    return { name, creds };
}
...
async function wcFetch(creds, path, init) {
    ...
    if (!res.ok)
        throw new Error(`WooCommerce ${res.status}: ${text.slice(0, 400)}`);
    return text ? JSON.parse(text) : {};
}
function requireApproval(args, tool) {
    if (args.approved !== true) {
        throw new Error(`${tool} requires approved:true after human review (run on staging first, then promote)`);
    }
}
function requireProductionApproval(envName, args, tool) {
...
export class WooCommercePlugin extends BasePlugin {
    name = "woocommerce";
    version = "0.2.0-beta";
    description = "Dual-env WooCommerce ops (staging + production). Test on staging; production writes need approval.";
        ...
        return [
            {
                ...
                description: "Show which store environments are configured (staging/production/local) and default. Never prints secrets.",
                parameters: { type: "object", properties: {} },
                handler: async (_args, context) => {
                    const config = readPluginConfig(context);
                    const envs = config.environments ?? {};
                    const summarize = (c) => c?.baseUrl
                    ...
                    return {
                        ...
                        policy: "Always apply and verify on staging before production. Production writes require approved:true.",
                    };
                },
            },
            {
            ...
            },
            {
                ...
                description: "Update product / Phantasy meta. Production requires approved:true. Prefer staging first.",
                parameters: {
                    type: "object",
                    properties: {
                        ...envParam,
                ...
                handler: async (args, context) => {
                    ...
                    try {
                        ...
                        const body = {};
                        if (a.regularPrice !== undefined)
                            body.regular_price = String(a.regularPrice);
            ...
            },
            {
                ...
                description: "Update order status. Production requires approved:true unless only staging.",
                parameters: {
                    type: "object",
                    properties: {
                        ...envParam,
            ...
            },
            {
                ...
                description: "APPROVAL REQUIRED always. Create simple product. Use staging first; production needs separate approved promote.",
                parameters: {
                    type: "object",
                    properties: {
                        ...envParam,
                ...
                handler: async (args, context) => {
                    ...
                    const meta_data = [
                        ...
                        {
                            key: "_phantasy_store_status",
                            value: String(a.storeStatus ?? (a.isPreorder ? "pre-order" : "live")),
                        },
                    ];
            ...
            },
            {
                ...
                handler: async (args, context) => {
                    ...
                    const order = (await wcFetch(creds, `/orders/${id}`));
            ...
            },
            {
                ...
                description: "Compare staging vs production health + order summary counts (read-only). Use before promote.",
                parameters: { type: "object", properties: {} },
                handler: async (_args, context) => {
                    const config = readPluginConfig(context);
                    const out = {};
